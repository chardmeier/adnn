...
namespace idx {
	...
	typedef mpl::vector2_c<int,0,4> I_P;
	...
	typedef mpl::vector2_c<int,1,3> I_R1;
	typedef mpl::vector2_c<int,1,4> I_R2;
	typedef mpl::vector2_c<int,1,5> I_R3;
...
auto load_nn6(const std::string &file, const classmap &classes, vocmap &srcvocmap, vocmap &antvocmap, const tagmap &tags, int nlink = -1) {
	...
	vocmap fulltgtvoc;
	// Note: we're assuming UNKNOWN_WORD has voc id 0 and voc_lookup will return
	// ids in increasing order without gaps.
	typedef std::pair<std::size_t,std::string> sizevoc_pair;
	typedef std::vector<sizevoc_pair> voccnt_vector;
	voccnt_vector tgtvoccnt;
	tgtvoccnt.push_back(std::make_pair(0, "<unk>"));
	...
	for(std::string line; getline(is, line);) {
		if(line.compare(0, 8, "ANAPHORA") == 0)
			...
			if(TrainingMode) {
				...
				std::string word;
				getline(ss, word, ' '); // get rid of ANTECEDENT tag
				while(getline(ss, word, ' ')) {
					voc_id v = voc_lookup(word, fulltgtvoc, true);
					if(v >= tgtvoccnt.size())
						tgtvoccnt.push_back(std::make_pair(1, word));
					else
						tgtvoccnt[v].first++;
				}
			}
	...
	if(TrainingMode) {
		const std::size_t TGTVOC_SIZE = 1000;
		voccnt_vector::const_iterator endit = tgtvoccnt.end();
		if(tgtvoccnt.size() > TGTVOC_SIZE) {
			std::sort(tgtvoccnt.begin(), tgtvoccnt.end(), std::greater<sizevoc_pair>());
			// If there are ties in the word frequency, make sure all items with the same frequency as
			// number 1000 get included.
			endit = tgtvoccnt.begin() + TGTVOC_SIZE - 1;
			std::size_t freq = endit->first;
			endit = std::find_if(endit, tgtvoccnt.cend(), [freq] (const sizevoc_pair &x) { return x.first < freq; });
		}
		for(voccnt_vector::const_iterator it = tgtvoccnt.begin(); it != endit; ++it)
			voc_lookup(it->second, antvocmap, true);
	}
	...
	for(std::size_t i = 0; i < nn6_lines.size(); i++) {
		...
		if(tag == "ANAPHORA") {
			...
			for(int j = 0; j < 7; j++) {
				...
				if(j == 3) { // the pronoun itself
					...
					if(word == "it")
						...
						std::cerr << "Unknown pronoun: " << word << ". Skipping example." << std::endl;
						skip_example = true;
						ex--;
						for(int k = 0; k < 3; k++)
							srcctx_triplets[k].pop_back();
					}
					P(ex,widx) = 1.0;
				} else { // context words
		...
		} else if(!skip_example && tag == "TARGET") {
			std::string word;
			bool found = false;
			while(getline(ss, word, ' ')) {
				boost::to_lower(word);
			...
			if(!skip_example) {
				if(tag == "NADA")
					...
					std::string word;
					while(getline(ss, word, ' ')) {
						voc_id v = voc_lookup(word, antvocmap, false); // don't add words to vocabulary at this point!
	...
	if(ex + 1 < nexmpl) {
		...
		P.conservativeResize(nexmpl, Eigen::NoChange);
	...
	// srcctx_triplets[4] isn't used because P is a dense matrix now.
	...
	return make_nn6_dataset(nlink,
		fusion::make_vector(fusion::make_vector(A, T, antmap, nada, P),
			boost::array<wordinput_type,6>({ L3, L2, L1, R1, R2, R3 }),
...
auto make_nn6(const Inputs &input,
	...
	int size_P = netops::at_spec<idx::I_P>(ispec).cols();
	int size_nada = netops::at_spec<idx::I_nada>(ispec).cols();
	...
	auto wspec = fusion::make_vector(
			...
			lweights<Float>(6 * size_srcembed + size_antembed + size_P + size_nada, size_hidden),
			lweights<Float>(size_hidden, size_output - 1));
...
auto nn6_dataset<InputSeq,Targets>::subset(std::size_t from, std::size_t to) const {
	...
	const auto &P = netops::at_spec<idx::I_P>(input_);
